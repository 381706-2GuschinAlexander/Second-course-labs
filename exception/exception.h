#ifndef EXP

#define __NEG_SIZE "Negative lenght/size"
#define __IND_IS_OUT_OF_RANGE "Index is out of range"
#define __STACK_IS_FULL "Attempt to put in full stack/queue/list" 
#define __STACK_IS_EMPTY "Attempt to get from empty stack/queue/list"
#define __DIFF_SIZE "Attempt to interact with object with different size" 
#define __INVALID_MATRIX "Invalid Object"
#define __INVALID_SIZE_CTL "Invalid size compare to length"


#endif // !EXP


