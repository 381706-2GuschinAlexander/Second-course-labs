...
void Text::Insert(const int pos, const char* string)
{
...
void Text::Insert(TTree* start, TTree* string)
{
	TTree* tmp1 = string;
	TTree* tmp2 = root;
	TTreeIterator iter(root);
...
int Text::Find(const char* string)
{
	TTreeIterator iter(root);
	int len = strlen(string);
	int i = 0;
...
TTree* Text::FindTree(const char* string)
{
	TTreeIterator iter(root);
	TTree* res = NULL;
	int len = strlen(string);
...
char* Text::Copy(const int start, const int len)
{
...
TTree* Text::Copy(TTree* start, const int len)
{
...
void Text::Delete(const int start_del, const int lenght)
{
...
void Text::Delete(TTree* start_del, const int lenght)
{
	TTreeIterator iter(root);
	int i = lenght;
	bool flag = false;
